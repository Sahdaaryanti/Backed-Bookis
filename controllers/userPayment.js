...
const createUserPayment = async (req, res) => {
  try {
    const newUserPayment = await userPayment.create(req.body);
    res.json(newUserPayment);
  } catch (error) {
    console.error(error);
...
const getUserPaymentById = async (req, res) => {
  try {
    const userPaymentData = await userPayment.findByPk(req.params.id);
    res.json(userPaymentData);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal Server Error' });
  }
...
const updateUserPayment = async (req, res) => {
  try {
    const updatedUserPayment = await userPayment.update(req.body, {
      where: { id: req.params.id },
    });
    res.json(updatedUserPayment);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal Server Error' });
  }
